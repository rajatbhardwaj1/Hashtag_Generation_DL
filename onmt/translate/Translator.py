...
class Translator(object):
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                if sent_number % 100 == 0:
                    print("Finish decoding for %d files and spend %.2f seconds" % (sent_number, time.time() - t0))
                ...
                if attn_debug:
                    ...
                    for word, row in zip(preds, attns):
                        ...
                        row_format = "{:>10.10} " + "{:>10.7f} " * len(srcs)
                    os.write(1, output.encode('utf-8'))
                    with open('vis.attention', 'w') as f:
                        item = dict()
                        item['source'] = srcs
                        item['translation'] = preds
                        item['attentions'] = [{'name': 'encoder_decoder_attention',
                                               'type': 'simple',
                                               'value': attns}]
                        res = {0: item}
                        import json
                    ...
                    with open('attention.json', 'w') as f:
                        item = dict()
                        item['article_lst'] = srcs
                        item['decoded_lst'] = preds
                        item['abstract_str'] = ' '.join(preds)
                        item['attn_dists'] = attns
                        item['p_gens'] = [0] * len(preds)
    ...
    def translate_batch(self, batch, data):
        ...
        memory_lengths0 = src_lengths.repeat(beam_size)
        memory_lengths1 = conversation_lengths.repeat(beam_size)
        ...
        for i in range(self.max_length):
            ...
            if self.model.__class__.__name__ == "NMTModel":
                dec_out, dec_states, attn = self.model.decoder(
                    inp, memory_bank, dec_states, memory_lengths=memory_lengths0) #(memory_lengths0, memory_lengths1)
            else:
                dec_out, dec_states, attn = self.model.decoder(
                    inp, memory_bank, dec_states,
                    memory_lengths=(memory_lengths0, memory_lengths1))  # (memory_lengths0, memory_lengths1)
            ...
            for j, b in enumerate(beam):
                if self.model.__class__.__name__ == "NMTModel":
                    memory_lengths_per_beam = memory_lengths0[j]
                else:
                    memory_lengths_per_beam = torch.max(memory_lengths0) + memory_lengths1[j]
                b.advance(out[:, j],
                          beam_attn.data[:, j, :memory_lengths_per_beam])
    ...
    def _run_target(self, batch, data):
        ...
        if self.model.__class__.__name__ == "NMTModel":
            dec_out, dec_states, attn = self.model.decoder(
                tgt_in, memory_bank, dec_states, memory_lengths=src_lengths)  # (memory_lengths0, memory_lengths1)
        else:
            dec_out, dec_states, attn = self.model.decoder(
                tgt_in, memory_bank, dec_states,
                memory_lengths=(src_lengths, conversation_lengths))
